...
trait HasRoles
{
    ...
    {
        $databaseRoles = $this->roles->merge($this->groups()->with('roles')->get());
        if (is_string($roles)) {
            return $databaseRoles->contains('name', $roles);
        }

        if ($roles instanceof Role) {
            return $databaseRoles->contains('id', $roles->id);
        }
    ...
    public function hasPermissionTo($permission)
    {
        ...
        return $this->hasDirectPermission($permission) || $this->hasPermissionViaRole($permission) || $this->hasPermissionViaGroup($permission);
    }
    ...
    /**
     * Determine if the user has, via groups, the given permission.
     *
     * @param Permission $permission
     *
     * @return bool
     */
    protected function hasPermissionViaGroup(Permission $permission)
    {
        return $this->hasRole($permission->groups);
    }
    ...
    {
        return $this->permissions->merge($this->getPermissionsViaRoles(), $this->getPermissionsViaGroups())->sort()->values();
    }
}
